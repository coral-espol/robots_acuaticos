...
Simulacion en ROS 2 Jazzy + Gazebo Harmonic de un enjambre de robots acuaticos de superficie (camaroneros), operando dentro de una piscina/camaronera simulada.

Actualmente se han probado corridas de hasta 20 robots en simultaneo.
...
Los paquetes sin sufijo y `model2` se mantienen solo como referencia historica y comparativa.
...
1. `cmd_vel_swarm`
   Boids 2D: separacion, cohesion, alineacion y geofence. Es una referencia de comportamiento de enjambre, no el algoritmo que corre en hardware real.
...
- `src/swarm_worlds`: mundos de Gazebo, incluida la camaronera estatica.
- `data/`: salidas generadas por corridas, como CSV y summaries.
...
Este repositorio esta preparado para versionar el codigo fuente y la configuracion del proyecto.

Se excluyen artefactos generados y configuraciones locales de editor como:

- `build/`, `install/`, `log/`
- salidas de `data/*.csv` y `data/*_summary.txt`
- `.vscode/`
- `CLAUDE.md`
